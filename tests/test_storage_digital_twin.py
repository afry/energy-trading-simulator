...
class TestStorageDigitalTwin(TestCase):
    ...
    storage_digital_twin = StorageDigitalTwin(max_capacity_kwh=100.0, max_charge_rate_fraction=0.25,
                                              max_discharge_rate_fraction=0.2, start_capacity_kwh=11.0,
    ...
    def test_charge_battery(self):
        ...
        # self.storage_digital_twin.charge(9)
        self.assertAlmostEqual(9, self.storage_digital_twin.charge(9.0))
    ...
    def test_discharge_battery(self):
        """Test discharging the battery with the input amount. Total discharging amount should be equal or less than
            (max_capacity_kwh * max_discharge_rate_fraction), if it does not exceed (capacity * discharging_efficiency),
             at one time step."""
        # self.storage_digital_twin.discharge(20)
        self.assertAlmostEqual(18.6, self.storage_digital_twin.discharge(20.0))

    def test_possible_charge_amount(self):
        # self.storage_digital_twin.get_possible_charge_amount()
        self.assertAlmostEqual(25, self.storage_digital_twin.get_possible_charge_amount())

    def test_possible_discharge_amount(self):
        # self.storage_digital_twin.get_possible_discharge_amount()
        self.assertAlmostEqual(1.30199999, self.storage_digital_twin.get_possible_discharge_amount())
