...
class TestGrossBid(TestCase):
    ...
    def test_co2_intensity_validation_sell(self):
        ...
        with self.assertRaises(ValueError):
            GrossBid(self.some_datetime, Action.SELL, Resource.ELECTRICITY, 200, math.inf, 'BuildingAgent', False, -0.1)
    ...
    def test_co2_intensity_validation_sell_none(self):
        ...
        with self.assertRaises(ValueError):
            GrossBid(self.some_datetime, Action.SELL, Resource.ELECTRICITY, 200, math.inf, 'BuildingAgent', False)

    def test_co2_intensity_validation_buy(self):
        """Test that an exception is thrown if a BUY bid is created with a co2_intensity"""
        with self.assertRaises(ValueError):
            GrossBid(self.some_datetime, Action.BUY, Resource.ELECTRICITY, 200, math.inf, 'BuildingAgent', False, 0.1)
