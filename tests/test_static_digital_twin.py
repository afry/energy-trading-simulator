...
from tradingplatformpoc.trading_platform_utils import datetime_indexed_value_series, datetime_array

FEB_1ST_1AM = datetime(2019, 2, 1, 1, 0, 0)


class TestStaticDigitalTwin(TestCase):
    datetime_array = datetime_array(datetime(2019, 2, 1, 1), datetime(2020, 1, 31, 23))
    ones_series = datetime_indexed_value_series(datetime_array, np.ones(shape=len(datetime_array)))
