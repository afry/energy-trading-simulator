...
class DistrictHeatingPrice:
    ...
    @classmethod
    def estimate_district_heating_price(cls, period: datetime.datetime) -> float:
        """
        ...
        * "Effect fee" based on the "peak day" of the month
        """
        effect_fee = cls.expected_effect_fee(period)
        jan_feb_extra = cls.marginal_grid_fee_assuming_top_bracket(period.year)
        base_marginal_price = cls.get_base_marginal_price(period.month)
    ...
    @classmethod
    def expected_effect_fee(cls, period: datetime.datetime) -> float:
        """
        ...
        """
        p = cls.probability_day_is_peak_day(period)
        return (cls.EFFECT_PRICE / 24) * p

    @classmethod
    def get_base_marginal_price(cls, month_of_year: int) -> float:
        """'Summer price' during May-September, 'Winter price' other months."""
        if 5 <= month_of_year <= 9:
            return cls.MARGINAL_PRICE_SUMMER  # Cheaper in summer
        else:
            return cls.MARGINAL_PRICE_WINTER

    @classmethod
    def marginal_grid_fee_assuming_top_bracket(cls, year: int) -> float:
        """
        ...
        return cls.GRID_FEE_MARGINAL_400_PLUS / hours_in_jan_feb

    @classmethod
    def exact_district_heating_price_for_month(cls, month: int, year: int, consumption_this_month_kwh: float,
        ...
        * "Effect fee" based on the "peak day" of the month
        ...
        """
        effect_fee = cls.exact_effect_fee(prev_month_peak_day_avg_consumption_kw)
        grid_fee = cls.get_grid_fee_for_month(jan_feb_avg_consumption_kw, year, month)
        base_marginal_price = cls.get_base_marginal_price(month)
        return base_marginal_price * consumption_this_month_kwh + effect_fee + grid_fee

    @classmethod
    def exact_effect_fee(cls, monthly_peak_day_avg_consumption_kw: float) -> float:
        """
        ...
        """
        return cls.EFFECT_PRICE * monthly_peak_day_avg_consumption_kw

    @classmethod
    def get_yearly_grid_fee(cls, jan_feb_hourly_avg_consumption_kw: float) -> float:
        """Based on Jan-Feb average hourly heating use."""
        if jan_feb_hourly_avg_consumption_kw < 50:
            return cls.GRID_FEE_FIXED_SUB_50 + cls.GRID_FEE_MARGINAL_SUB_50 * jan_feb_hourly_avg_consumption_kw
        elif jan_feb_hourly_avg_consumption_kw < 100:
            return cls.GRID_FEE_FIXED_50_100 + cls.GRID_FEE_MARGINAL_50_100 * jan_feb_hourly_avg_consumption_kw
        elif jan_feb_hourly_avg_consumption_kw < 200:
            return cls.GRID_FEE_FIXED_100_200 + cls.GRID_FEE_MARGINAL_100_200 * jan_feb_hourly_avg_consumption_kw
        elif jan_feb_hourly_avg_consumption_kw < 400:
            return cls.GRID_FEE_FIXED_200_400 + cls.GRID_FEE_MARGINAL_200_400 * jan_feb_hourly_avg_consumption_kw
        else:
            return cls.GRID_FEE_FIXED_400_PLUS + cls.GRID_FEE_MARGINAL_400_PLUS * jan_feb_hourly_avg_consumption_kw

    @classmethod
    def get_grid_fee_for_month(cls, jan_feb_hourly_avg_consumption_kw: float, year: int, month_of_year: int) -> float:
        """
        The grid fee is based on the average consumption in kW during January and February.
        ...
        yearly_fee = cls.get_yearly_grid_fee(jan_feb_hourly_avg_consumption_kw)
...
class HeatingPrice(IPrice):
    ...
    def get_estimated_retail_price(self, period: datetime.datetime, include_tax: bool) -> float:
        ...
        return DistrictHeatingPrice.estimate_district_heating_price(period)
    ...
    def get_estimated_wholesale_price(self, period: datetime.datetime) -> float:
        ...
        why this method is named 'estimated'.
        """
        return DistrictHeatingPrice.estimate_district_heating_price(period) * self.heating_wholesale_price_fraction
    ...
    def get_exact_retail_price(self, period: datetime.datetime, include_tax: bool) -> float:
        ...
        if consumption_this_month_kwh == 0:
            return DistrictHeatingPrice.handle_no_consumption_when_calculating_heating_price(period)
        jan_feb_avg_consumption_kw = DistrictHeatingPrice.calculate_jan_feb_avg_heating_sold(
            self.all_external_heating_sells, period)
        prev_month_peak_day_avg_consumption_kw = DistrictHeatingPrice.calculate_peak_day_avg_cons_kw(
            self.all_external_heating_sells, period.year, period.month)
        total_cost_for_month = DistrictHeatingPrice.exact_district_heating_price_for_month(
