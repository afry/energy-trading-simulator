...
if len(ids) > 0:
    ...
    if choosen_config_id_to_view is not None:
        st.session_state.choosen_id_to_view = {'config_id': choosen_config_id_to_view,
                                               'job_id': ids[choosen_config_id_to_view]}
        with st.expander('Configuration *{}* in JSON format'.format(st.session_state.choosen_id_to_view['config_id'])):
            st.json(read_config(st.session_state.choosen_id_to_view['config_id']), expanded=True)
else:
