...
for j in range(n_agents):
    mod[j], res[j] = solve_model(pyo.SolverFactory('gurobi', solver_io="python", executable=r"C:\gurobi1003\win64"),
                                 summer_mode=True, month=7, agent=j,
                                 external_elec_buy_price=price_buy, external_elec_sell_price=price_sell,
                                 external_heat_buy_price=Hprice_energy, battery_capacity=Emax_BES[j],
                                 battery_charge_rate=Pmax_BES_Cha[j], battery_discharge_rate=Pmax_BES_Dis[j],
                                 SOCBES0=SOCBES0[j],
                                 HP_Cproduct_active=HP_Cproduct_active[j], heatpump_COP=COPhp[j],
                                 heatpump_max_power=Phpmax[j],
                                 heatpump_max_heat=Hhpmax[j], booster_heatpump_COP=COPhpB[j],
                                 booster_heatpump_max_power=PhpBmax[j], booster_heatpump_max_heat=HhpBmax[j],
                                 build_area=build_area[j], SOCTES0=SOCTES0[j],
                                 ...
                                 hot_water_heatdem=Hhw.iloc[j, :],
                                 space_heating_heatdem=Hsh.iloc[j, :], excess_heat=hsh_excess_df.iloc[j, :],
                                 cold_consumption=Cld.iloc[j, :],
                                 pv_production=Ppv.iloc[j, :], battery_efficiency=eff,
                                 max_elec_transfer_between_agents=500,
                                 max_elec_transfer_to_external=1000, max_heat_transfer_between_agents=500,
                                 max_heat_transfer_to_external=1000, chiller_COP=1.5, Pccmax=100,
                                 thermalstorage_efficiency=0.98,
                                 heat_trans_loss=0.05, cold_trans_loss=0.05, trading_horizon=24)
