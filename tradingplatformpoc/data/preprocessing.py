...
def read_energy_data(data_path: str = "tradingplatformpoc.data",
    ...
    energy_data['coop_high_heat_consumed'] = np.maximum(energy_data['coop_hw_consumed_kwh'], 0)
    energy_data['coop_low_heat_consumed'] = np.maximum(energy_data['coop_net_space_heat_consumed'], 0)
    return energy_data[['coop_electricity_consumed', 'coop_high_heat_consumed', 'coop_low_heat_consumed']].reset_index()
