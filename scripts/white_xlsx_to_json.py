...
def get_battery_size_kwh(pv_areas: pd.Series) -> pd.Series:
    """
    Battery size should be proportional to PV area.
    To dimension batteries so that their capacity is equal to the max production of solar panels in an hour, we
    calculate the max PV production per square meter, and then multiply this by the total PV panel area.
    """
    return (pv_areas * MAX_KW_PV_PROD_PER_SQM).round().astype(float)
